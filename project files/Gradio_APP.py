...
def get_user_based(student_id, limit=3):
   try:
        from UserBased import recommend_and_log
        engine = get_engine()
        
        recommendations = recommend_and_log(student_id)
        question_ids = [rec['question_id'] for rec in recommendations][:limit]

        if not question_ids:
            return pd.DataFrame()

        query = text("SELECT * FROM questions WHERE id IN :ids")
        df = pd.read_sql_query(query, engine, params={"ids": tuple(question_ids)})
        df['source'] = 'user_based'
        return df
   except Exception as e:
        print(f"User-based recommendation error: {str(e)}")
        return pd.DataFrame()
